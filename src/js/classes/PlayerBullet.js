...
export class PlayerBullet {
    constructor(scene, position, game) {
        ...
        const material = new THREE.MeshStandardMaterial({
            map: new THREE.TextureLoader().load('/assets/textures/player_texture.jpg'),
            roughness: 0.5,
            metalness: 0.2
        });
    ...
    handleCollision(collidedMesh) {
        ...
        if (collidedType === 'alien' || collidedType === 'mothership') {
            // Always call the parent hit() method
            const destroyed = collidedMesh.userData.parent?.hit();
            if (destroyed) {
                new Explosion(this.scene, this.mesh.position, 10, 1); // Full explosion
                this.game.playSound('alienExplosion');
            } else {
                // Visual feedback for hit
                collidedMesh.rotation.x += (Math.random() - 0.5) * 0.3;
                new Explosion(this.scene, this.mesh.position, 5, 0.5); // Smaller explosion
                this.game.playSound('alienExplosion');
            }
            this.destroyBullet();
